...
const LogsTable = () => {
    ...
    const getQueryErrorMessage = (error, fallback) => {
    ...
    const fetchData = async () => {
        const now = Date.now();
        if (loadingRef.current || now - lastSubmitAtRef.current < QUERY_COOLDOWN) {
            return;
        }
        ...
        const prevTabData = tabData[currentTabKey] || { totalGranted: 0, totalUsed: 0, totalAvailable: 0, unlimitedQuota: false, expiresAt: 0, tokenName: '', logs: [], tokenValid: false };
        const newTabData = { ...prevTabData, logs: Array.isArray(prevTabData.logs) ? prevTabData.logs : [] };
        ...
        try {
            ...
            if (process.env.REACT_APP_SHOW_BALANCE === "true") {
                try {
                    ...
                    if (usageData.code) {
                        ...
                        showQueryError(usageData.message || '查询令牌信息失败');
                    }
                } catch (e) {
                    showQueryError(getQueryErrorMessage(e, '查询令牌信息失败，请检查令牌是否正确'));
                }
            }
            ...
            if (process.env.REACT_APP_SHOW_DETAIL === "true") {
                try {
                    ...
                    if (success) {
                        newTabData.logs = Array.isArray(logData) ? [...logData].reverse() : [];
                        hasValidData = true;
                    } else {
                        showQueryError('查询调用详情失败，请稍后重试');
                    }
                } catch (e) {
                    showQueryError(getQueryErrorMessage(e, '查询调用详情失败，请稍后重试'));
                }
            }

            if (hasValidData && requestIdRef.current === currentRequestId) {
                setTabData((prevData) => ({
                    ...prevData,
                    [currentTabKey]: newTabData,
                }));
                setActiveKeys(['1', '2']);
            }
        } finally {
